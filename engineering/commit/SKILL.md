---
name: commit
description: Stage and commit the current changes following the repo's commit message conventions. Use when the user asks to commit, says 'ship it' / 'save this', or wants work checkpointed with a proper message.
...
If the user passed a message along with the invocation, use it as the commit message (but still format it to match the repo's style -- add the correct prefix, fix casing, etc.).
