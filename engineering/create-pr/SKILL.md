---
name: create-pr
description: Open a pull request whose title and description follow the repo's conventions. Use when the user asks for a PR, wants a branch reviewed, or says 'open a PR' / 'send this for review'.
...
If the user passed text along with the invocation, use it as the basis for the title (but reformat to match the repo's convention).
