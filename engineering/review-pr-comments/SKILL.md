...
description: Read a PR's review comments, judge each one, implement only the good suggestions, and reply with the decision. Use when the user asks to handle, address, or respond to PR feedback or review comments.
...
If the user passed a PR number along with the invocation, use that instead.
