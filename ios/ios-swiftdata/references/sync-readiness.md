...
- Signed out / no account: local-first continues untouched; sync is
  idle. Nothing about durability may depend on an account
  existing.
...
- **Escalations, in order of weight**, only if debug builds fabricate
  records into the real on-device store: partition the store
